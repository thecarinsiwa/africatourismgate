...
type DestinationHeroImageSectionProps = {
  destinationId: string;
  imageUrl: string | null;
  onSaved: (destination: Destination) => void;
};

export function DestinationHeroImageSection({
  destinationId,
  imageUrl,
  onSaved,
}: DestinationHeroImageSectionProps) {
  const { destinations: getDestinationsErrorMessage } = useAdminErrorMessages();
  const t = useTranslations('modules.destinations.heroImage');
  ...
  const saveHero = useCallback(
    async (nextUrl: string | null) => {
      ...
      setError(null);
      try {
        const updated = await getApiClient().updateDestination(destinationId, {
          imageUrl: nextUrl,
        });
        setUrlInput(nextUrl ?? '');
        onSaved(updated);
      } catch (err) {
        setError(getDestinationsErrorMessage(err));
      } finally {
        setSaving(false);
      }
    },
    [destinationId, getDestinationsErrorMessage, onSaved],
  );

  async function handleHeroImagePick(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const response = await fetch(
        `${resolveApiBaseUrl()}/destinations/${destinationId}/upload-image`,
        {
          method: 'POST',
      ...
      if (!response.ok) {
        throw new Error('Upload hero image failed');
      }
      const payload = (await response.json()) as { url?: string };
      if (!payload.url) {
        throw new Error('Invalid upload response');
      }
      await saveHero(payload.url);
    } catch {
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              <input
                ...
                className="hidden"
                onChange={(e) => void handleHeroImagePick(e)}
