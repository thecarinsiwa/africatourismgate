...
import {
  downloadVehiclesBookingsPdf,
  downloadVehiclesCatalogPdf,
  downloadVehiclesKpiSummaryPdf,
  downloadVehiclesWorkbook,
} from '../../lib/vehicles-reports';

type ExportAction = 'excel' | 'pdfKpi' | 'pdfCatalog' | 'pdfBookings';

type VehiclesExportDialogProps = {
  open: boolean;
...
export function VehiclesExportDialog({
  open,
  onOpenChange,
  search,
  agencyId,
  categoryId,
  canExportBookings,
}: VehiclesExportDialogProps) {
  const { locations: getLocationsErrorMessage } = useAdminErrorMessages();
  const t = useTranslations('modules.locations.exports');
  ...
  const runExport = useCallback(
    async (action: ExportAction, runner: () => Promise<void>) => {
      ...
      try {
        ...
        toast({
          variant: 'error',
          message: getLocationsErrorMessage(error) || t('error'),
        });
      } finally {
        resetLoading();
      }
    },
    [getLocationsErrorMessage, hasValidPeriod, onOpenChange, resetLoading, t, toast],
  );
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          <Button
            ...
            onClick={() =>
              void runExport('excel', () => downloadVehiclesWorkbook(datedScope))
            }
          >
            {t('excel')}
          </Button>
          <Button
            ...
            onClick={() => void runExport('pdfKpi', () => downloadVehiclesKpiSummaryPdf(scope))}
          >
            {t('pdfKpi')}
          </Button>
          <Button
            ...
            onClick={() => void runExport('pdfCatalog', () => downloadVehiclesCatalogPdf(scope))}
          >
            {t('pdfCatalog')}
          </Button>
          {canExportBookings ? (
            <Button
              ...
              onClick={() =>
                void runExport('pdfBookings', () => downloadVehiclesBookingsPdf(datedScope))
              }
            >
              {t('pdfBookings')}
            </Button>
