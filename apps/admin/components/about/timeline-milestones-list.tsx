...
export function TimelineMilestonesList() {
  const { about: getAboutErrorMessage } = useAdminErrorMessages();
  const t = useTranslations('modules.about.timeline.list');
  ...
  const [statusFilter, setStatusFilter] = useState<'' | AboutTimelineMilestoneStatus>('');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await getApiClient().listAboutTimelineMilestones({
        page,
        limit: PAGE_SIZE,
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div>
            ...
            <p className="text-sm text-atg-muted">{row.original.year}</p>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <div>
            <p className="text-sm text-atg-fg">{row.original.periodLabel}</p>
            <p className="text-sm text-atg-muted">{row.original.periodTitle}</p>
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <DataTableActions>
            <DataTableActionButton
              action="edit"
              href={`/contenu/a-propos/timeline/${row.original.id}`}
            />
            {canWrite ? (
              <DataTableActionButton
                action="delete"
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-end">
          ...
          <select
            id={statusFilterId}
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as '' | AboutTimelineMilestoneStatus)}
