...
export function GapMediaStatCards({ className }: { className?: string }) {
  ...
  const t = useTranslations('modules.gap.media.stats');
  ...
  useEffect(() => {
    ...
    async function loadKpi(key: ContentStatKpiKey): Promise<KpiCardState> {
      try {
        if (key === 'total') {
          const result = await client.listGapMediaItems({ page: 1, limit: 1 });
          return { status: 'ready', displayValue: formatCount(result.meta.total) };
        }
        if (key === 'published') {
          const result = await client.listGapMediaItems({
            page: 1,
            limit: 1,
            status: 'published',
          });
        ...
        if (key === 'draft') {
          const result = await client.listGapMediaItems({ page: 1, limit: 1, status: 'draft' });
          return { status: 'ready', displayValue: formatCount(result.meta.total) };
        }
        const result = await client.listGapMediaItems({ page: 1, limit: 1, locale: 'fr' });
        return { status: 'ready', displayValue: formatCount(result.meta.total) };
      } catch (error) {
        return {
          status: 'error',
