...
type FundEntryAttachmentsSectionProps = {
  fundEntryId: string;
...
export function FundEntryAttachmentsSection({
  fundEntryId,
  initialAttachments = [],
  canWrite,
  onChanged,
}: FundEntryAttachmentsSectionProps) {
  const t = useTranslations('modules.treasury.entries.form.attachments');
  ...
  const refresh = useCallback(async () => {
    const list = await getApiClient().listFundEntryAttachments(fundEntryId);
    setAttachments(list);
    onChanged?.(list);
  }, [fundEntryId, onChanged]);
  ...
  const handleUpload = useCallback(
    async (file: File | undefined) => {
      ...
      try {
        ...
        await getApiClient().uploadFundEntryAttachment(fundEntryId, body);
        await refresh();
      } catch (err) {
        setError(
    ...
    [canWrite, commonErrors, fundEntryId, refresh, t],
  );
  ...
  const handleRemove = useCallback(
    async (attachmentId: string) => {
      ...
      try {
        await getApiClient().deleteFundEntryAttachment(fundEntryId, attachmentId);
        await refresh();
      } catch (err) {
        setError(
          resolveUnknownApiError(err, commonErrors, { useParseApiMessage: true }),
    ...
    [canWrite, commonErrors, fundEntryId, refresh],
  );
          ...
          {attachments.map((item) => (
            ...
            >
              ...
              {canWrite ? (
                ...
                >
                  {removingId === item.id ? t('uploading') : t('remove')}
                </Button>
              ) : null}
            </li>
          ))}
