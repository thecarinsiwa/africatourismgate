...
type PromoCodeCoverImageSectionProps = {
  promoCodeId: string;
  coverImageUrl: string | null;
  onSaved: (promoCode: PromoCode) => void;
};

export function PromoCodeCoverImageSection({
  promoCodeId,
  coverImageUrl,
  onSaved,
}: PromoCodeCoverImageSectionProps) {
  const { promoCodes: getPromoCodesErrorMessage } = useAdminErrorMessages();
  const t = useTranslations('modules.promoCodes.coverImage');
  ...
  const saveCover = useCallback(
    async (nextUrl: string | null) => {
      ...
      setError(null);
      try {
        const updated = await getApiClient().updatePromoCode(promoCodeId, {
          coverImageUrl: nextUrl,
        });
        setUrlInput(nextUrl ?? '');
        onSaved(updated);
      } catch (err) {
        setError(getPromoCodesErrorMessage(err));
      } finally {
        setSaving(false);
      }
    },
    [getPromoCodesErrorMessage, onSaved, promoCodeId],
  );
  ...
  async function handleCoverImagePick(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { url } = await getApiClient().uploadPromoCodeImage(promoCodeId, body);
      await saveCover(url);
    } catch {
