...
export class RolePermissionsService {
  ...
  async findAll(query: PaginationQueryDto): Promise<PaginatedResult<RolePermissions>> {
  ...
  async create(dto: Partial<RolePermissions>): Promise<RolePermissions> {
  ...
  async remove(roleId: string, permissionId: string): Promise<void> {
    const row = await this.repository.findOne({
      where: { roleId, permissionId } as never,
    });
