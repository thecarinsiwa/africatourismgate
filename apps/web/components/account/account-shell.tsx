...
export function AccountShell({ children }: Props) {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      <nav
        className="mb-6 flex flex-wrap items-center gap-2 text-sm text-atg-muted"
      ...
      >
        ...
        <span className="font-medium text-atg-fg">{t.account.title}</span>
        {pathname !== '/account' &&
        pathname !== '/account/profile' &&
        !pathname.match(/^\/account\/reservations\/[^/]+$/) ? (
          <>
            <span aria-hidden>/</span>
            <span className="text-atg-fg/80">{pageTitle}</span>
          </>
        ) : null}
      </nav>

      <div className="mb-6">
        <h1 className="text-2xl font-bold text-atg-fg sm:text-3xl">
          {t.account.title}
        </h1>
        <p className="mt-1 text-sm text-atg-muted">{t.account.subtitle}</p>
      </div>

      <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
        <aside className="w-full shrink-0 lg:sticky lg:top-6 lg:w-60">
          <nav
            className="rounded-xl border border-atg-border bg-atg-elevated p-2 shadow-sm dark:border-atg-border dark:bg-atg-elevated"
          ...
          >
            ...
            <div className="mt-2 border-t border-atg-border pt-2 dark:border-atg-border">
              <Link
                href="/hotels"
