...
export function PackageMixedBookingSidebar({
  detail,
  t,
}: Pick<PackageBookingSidebarProps, 'detail' | 't'>) {
  return (
    <aside
      id="reserve"
    ...
    >
      ...
      <div className="mt-6 space-y-4 border-t border-gray-100 pt-4 dark:border-atg-border">
        <p className="text-sm text-gray-600 dark:text-atg-muted">{t.mixedConfigureHint}</p>
        <p className="text-sm text-amber-800 dark:text-amber-200">{t.mixedCheckoutDisabled}</p>
        <a
          href="#items"
          className="inline-flex w-full min-h-[48px] items-center justify-center rounded-lg bg-primary px-6 py-3 text-sm font-bold uppercase tracking-wide text-white transition-colors hover:bg-primary-hover"
        >
          {t.configurePackage}
        </a>
      </div>
...
export function PackageMixedBookingMobileBar({
  detail,
  t,
}: Pick<PackageBookingSidebarProps, 'detail' | 't'>) {
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white/95 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] backdrop-blur-md dark:border-atg-border dark:bg-atg-elevated/95 lg:hidden pb-safe">
      <div className="mx-auto flex max-w-lg items-center gap-4">
        ...
        <a
          href="#items"
          className="min-h-[48px] shrink-0 rounded-lg bg-primary px-6 py-3 text-sm font-bold uppercase tracking-wide text-white hover:bg-primary-hover"
        >
          {t.configurePackage}
        </a>
      </div>
