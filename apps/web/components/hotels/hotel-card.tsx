      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <div className="mb-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-atg-muted">{hotel.location}, {hotel.country}</p>
          </div>
          <div className="flex flex-col items-end gap-1">
            ...
            <span className="text-xs text-gray-400 dark:text-atg-muted">{hotel.reviews} {t.reviews}</span>
          </div>
        </div>

        <div className="mb-3 flex flex-wrap items-center gap-3">
          <StarRow count={hotel.stars} />
          <span className="text-xs text-gray-400 dark:text-atg-muted">{hotel.stars} {t.stars}</span>
          <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">{t.freeCancel}</span>
        </div>

        <div className="mb-4 flex flex-wrap gap-2">
          {hotel.amenities.map((a) => (
            <span key={a} className="inline-flex items-center gap-1 rounded-md bg-gray-50 px-2 py-1 text-xs text-gray-600 dark:bg-white/5 dark:text-atg-muted" title={t.amenities[a]}>
              {AMENITY_ICONS[a]}<span className="hidden sm:inline">{t.amenities[a]}</span>
            </span>
          ))}
        </div>
        ...
        <div className="mt-auto flex flex-wrap items-end justify-between gap-4 border-t border-gray-100 pt-4 dark:border-atg-border">
          <div>
            <p className="text-2xl font-bold text-[#0f1a16] dark:text-white">${hotel.price}<span className="text-sm font-normal text-gray-500 dark:text-atg-muted"> {t.perNight}</span></p>
          </div>
          <div className="flex gap-2">
            <Link href={`/hotels/${hotel.slug}`} className="inline-flex min-h-[44px] items-center rounded-lg border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 transition-colors hover:border-primary hover:text-primary dark:border-atg-border dark:text-white/80 dark:hover:border-primary dark:hover:text-white">
              {t.viewDetails}
            </Link>
            <Link href={`/booking?propertyId=${encodeURIComponent(hotel.id)}`} className="inline-flex min-h-[44px] items-center rounded-lg bg-primary px-5 py-2 text-sm font-bold uppercase tracking-wide text-white transition-colors hover:bg-primary-hover">
              {t.bookNow}
            </Link>
          </div>
        </div>
