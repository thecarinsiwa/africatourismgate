...
import { logoutAuth, touchSession, unlockSession } from '../../lib/api/auth';
import {
  AUTH_CHANGED_EVENT,
  authTokensToWebSession,
  clearWebAuthState,
  getWebSession,
  saveWebSession,
...
export function WebSessionIdleLock() {
  ...
  const syncLockedState = useCallback(() => {
    const shouldLock =
      Boolean(getWebSession()?.accessToken) &&
      (isSessionLocked() || isIdleExpired());
    if (shouldLock) {
  ...
  const handleUnlock = useCallback(async () => {
    ...
    try {
      const tokens = await unlockSession({
        password,
        refreshToken: session.refreshToken,
      });
      saveWebSession(authTokensToWebSession(tokens, session.user));
      markActivity();
  ...
  const handleLogout = useCallback(async () => {
    const refreshToken = getWebSession()?.refreshToken;
    clearWebAuthState();
    if (refreshToken) {
      try {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="web-session-lock-title"
    >
      <div className="w-full max-w-md rounded-2xl border border-atg-border bg-atg-elevated p-6 shadow-lg dark:border-atg-border dark:bg-atg-elevated">
        <h1 id="web-session-lock-title" className="text-xl font-bold text-atg-fg">
          {t('title')}
        </h1>
        ...
        >
          <PasswordInput
            id="web-session-unlock-password"
            name="password"
