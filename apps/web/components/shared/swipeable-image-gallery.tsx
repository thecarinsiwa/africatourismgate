...
export type SwipeableGalleryImage = {
  id: string;
  url: string;
  caption?: string | null;
  sortOrder: number;
...
export type SwipeableGalleryLabels = {
  ariaLabel: string;
  openLightbox: string;
  close: string;
  previous: string;
...
type SwipeableImageGalleryProps = {
  images: SwipeableGalleryImage[];
  name: string;
  labels: SwipeableGalleryLabels;
};
        ...
        >
          {hasMultiple ? (
            <button
              type="button"
              onClick={goPrevious}
              className="absolute left-2 z-10 inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg border border-white/20 bg-black/40 text-2xl text-white backdrop-blur-sm transition-colors hover:bg-black/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:left-0"
          ...
          {hasMultiple ? (
            <button
              type="button"
              onClick={goNext}
              className="absolute right-2 z-10 inline-flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg border border-white/20 bg-black/40 text-2xl text-white backdrop-blur-sm transition-colors hover:bg-black/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:right-0"
...
export function SwipeableImageGallery({ images, name, labels }: SwipeableImageGalleryProps) {
  ...
  return (
    <section aria-label={labels.ariaLabel} className="min-w-0">
      <button
        type="button"
        onClick={() => {
      ...
      >
        ...
        {active.caption ? (
          <p className="absolute bottom-3 left-3 max-w-[70%] truncate rounded-md bg-atg-fg/70 px-2 py-1 text-xs text-atg-elevated backdrop-blur-sm">
            {active.caption}
          </p>
        ) : null}
      </button>
      ...
      {sorted.length > 1 ? (
        <div className="mt-3 flex max-w-full gap-2 overflow-x-auto pb-1 snap-x snap-mandatory [-webkit-overflow-scrolling:touch]">
