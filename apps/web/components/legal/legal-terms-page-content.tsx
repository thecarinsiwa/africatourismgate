...
export function LegalTermsPageContent() {
  ...
  const heroTitle = page?.title ?? legal.termsOfUseTitle;
  const heroDescription = legal.termsOfUseSubtitle;

  if (loading) {
    return (
      <LegalShell title={legal.termsOfUseTitle} description={heroDescription}>
  ...
  return (
    <LegalShell title={heroTitle} description={heroDescription}>
      <article
        ref={ref}
