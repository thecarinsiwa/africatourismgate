...
export async function uploadBookingPaymentProof(
  bookingId: string,
  file: File,
  paymentMethod: BookingPaymentProofMethod = 'bank_transfer',
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/bookings/${encodeURIComponent(bookingId)}/payment-proofs`,
    {
      method: 'POST',
  ...
  return (await res.json()) as BookingPaymentProof;
}

export async function fetchBookingPaymentProofBlob(
  bookingId: string,
  proofId: string,
): Promise<Blob> {
  const accessToken = await ensureClientAccessToken();
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/bookings/${encodeURIComponent(bookingId)}/payment-proofs/${encodeURIComponent(proofId)}/file`,
    { headers: { Authorization: `Bearer ${accessToken}` } },
  );
  if (!res.ok) {
